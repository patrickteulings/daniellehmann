...
class daniellehmann_Dark_Mode
{
  ...
  public function __construct()
  {
    ...
    // Add HTML classes.
    add_filter('daniellehmann_html_classes', array($this, 'html_classes'));
  ...
  public function customizer_controls($wp_customize)
  {
    ...
    if (is_object($colors_section)) {
      $colors_section->title = __('Colors & Dark Mode', 'daniellehmann');
    }
    ...
    $wp_customize->add_control(
      new daniellehmann_Customize_Notice_Control(
        $wp_customize,
        'respect_user_color_preference_notice',
        array(
          'section'         => 'colors',
          ...
          'active_callback' => function () {
            return 127 >= daniellehmann_Custom_Colors::get_relative_luminance_from_hex(get_theme_mod('background_color', 'D1E4DD'));
          },
        )
      )
    );
    ...
    $description .= sprintf(
      ...
      __('Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'daniellehmann'),
      esc_url(__('https://wordpress.org/support/article/twenty-twenty-one/#dark-mode-support', 'daniellehmann'))
    );
    $description .= '</p>';
    $description .= '<p>' . __('Dark Mode can also be turned on and off with a button that you can find in the bottom right corner of the page.', 'daniellehmann') . '</p>';
    ...
    $wp_customize->add_control(
      ...
      array(
        'type'            => 'checkbox',
        'section'         => 'colors',
        'label'           => esc_html__('Dark Mode support', 'daniellehmann'),
        'priority'        => 110,
  ...
  public function html_classes($classes)
  {
    ...
    $background_color            = get_theme_mod('background_color', 'D1E4DD');
    $should_respect_color_scheme = get_theme_mod('respect_user_color_preference', false);
    if ($should_respect_color_scheme && 127 <= daniellehmann_Custom_Colors::get_relative_luminance_from_hex($background_color)) {
  ...
  public function switch_should_render()
  {
    ...
    return (get_theme_mod('respect_user_color_preference', false) &&
      !$is_IE &&
      127 <= daniellehmann_Custom_Colors::get_relative_luminance_from_hex(get_theme_mod('background_color', 'D1E4DD')));
  }
  ...
  public function the_html($attrs = array())
  {
    ...
    printf(
      ...
      esc_html__('Dark Mode: %s', 'daniellehmann'),
...
?>
    <style>
      ...
      #dark-mode-toggler>span::before {
        content: '<?php esc_attr_e('Off', 'daniellehmann'); ?>';
      }

      #dark-mode-toggler[aria-pressed="true"]>span::before {
        content: '<?php esc_attr_e('On', 'daniellehmann'); ?>';
      }
...
<?php
  ...
  public function add_privacy_policy_content()
  {
    ...
    $content = '<p class="privacy-policy-tutorial">' . __('Twenty Twenty-One uses LocalStorage when Dark Mode support is enabled.', 'daniellehmann') . '</p>'
      . '<strong class="privacy-policy-tutorial">' . __('Suggested text:', 'daniellehmann') . '</strong> '
      . __('This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'daniellehmann');
